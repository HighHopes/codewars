...
def alias_gen(f_name, l_name):
    r = ""
    if f_name[0].upper() in FIRST_NAME and f_name[0].isalpha():
        r += FIRST_NAME[f_name[0].upper()]
    else:
        return "Your name must start with a letter from A - Z."

    r += " "

    if l_name[0].upper() in SURNAME and l_name[0].isalpha():
        r += SURNAME[l_name[0].upper()]
    else:
        return "Your name must start with a letter from A - Z."
